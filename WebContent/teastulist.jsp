	...
	$('form').submit(function() {
		...
		$.ajax({
				...
				'success' : function(data, textStatus,jqXHR) {	
					target.html("<table><tr><th>순서</th><th>이름</th><th>전화번호</th></tr>");
...
<body>
	...
	<input type="text" id="classid" value="${classid}" hidden="hidden">
		<table>
			...
			<c:if test="${empty teastulist}">
				<tr>
					<td colspan="2">수강생이 없습니다.</td>
				</tr>
			</c:if>
			<c:forEach items="${teastulist }" var="bean">
				<tr class="bean">
					<td><%=++cnt %></td>
					<td>${bean.name}</td>
